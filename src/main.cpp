...
int main(int, char**)
{
    ...
    ImFont* font = io.Fonts->AddFontFromFileTTF("c:\\Windows\\Fonts\\msyh.ttc", 25.0f, nullptr, io.Fonts->GetGlyphRangesChineseFull()); // 支持简体和繁体
    ...
    while (!glfwWindowShouldClose(window))
    {
...
#else
        bool b = true;
        ImGui::Begin(u8"WndName: 菜单", &b); // 这个应该也是传出参数
        ...
        if (c) {
            ImGui::Begin(u8"点击了", &b);
            ImGui::End();
        }

        bool r;
        ImGui::RadioButton("RadioButton", &r);

        static int multi;
        ImGui::RadioButton("RadioButton Combine 0", &multi, 0);
        ImGui::RadioButton("RadioButton Combine 1", &multi, 1);
        ImGui::RadioButton("RadioButton Combine 2", &multi, 2);
